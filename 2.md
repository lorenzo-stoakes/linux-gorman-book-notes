...
* Each of these banks is called a 'node', represented by `struct pglist_data`
  _even if the arch is UMA_.
...
* Each node is divided into blocks called zones, which represent ranges of
  memory, described by `struct zone_struct`, typedef-d to `zone_t`, one of
  `ZONE_DMA`, `ZONE_NORMAL` or `ZONE_HIGHMEM`.
...
* Each node is described by a `pg_data_t`, which is a `typedef` for `struct`
...
* `node_start_mapnr` - Page offset within global `mem_map`. Calculated in
  `free_area_init_core()` by determining number of pages between `mem_map` and
  local `mem_map` for this node called `lmem_map`.
